...
### 🔴 严格按报告格式输出（2026.04.27 更新）
- **绝对禁止**对报告内容做任何形式的简化、汇总、重组或省略
- **必须完整输出**原始报告中的所有设备明细行，每台设备的执行时长、异常类型、所有超链接（Crasheye/系统日志/游戏日志/Perfeye）一个都不能少
- **禁止自行总结汇总表**：不允许在报告正文外加汇总表格、统计图表或精简视图
- **结构必须一致**：保持 一/二/三 平台分级 → 版本行 → 任务行 → 设备明细行的缩进层级不变
- **原因**：用户需要一致、完整的报告格式来快速对比和查阅数据，任何简化都会导致信息缺失和理解成本增加

...
def main():
    ...
    print("=" * 70)
    print("创建并执行: 星砂 Xbox 端稳定性任务 (pipelineId=953)")
    print("自动选择最新 Xbox release 版本")
    print("=" * 70)
    print()

    # 1. 获取包列表，找到最新 Xbox release 版本
    print("1. 获取包列表...")
    ...
    # 过滤 Xbox 平台的 release 类型
    xbox_release_packages = []
    for pkg in packages:
        package_name = pkg.get('packageName', '').lower()
        download_url = pkg.get('downloadUrl', '')
        # 平台筛选: 路径包含 XBOX 或 xbox 或 xboxx
        if ('XBOX' in download_url or 'xbox' in download_url) and 'release' in package_name:
            # 解析版本信息，获取编译时间
            create_time = pkg.get('createTime', '')
            xbox_release_packages.append({
    ...
    if not xbox_release_packages:
        print("未找到 Xbox release 类型包!")
        sys.exit(1)

    # 按创建时间排序，最新的排在前面
    xbox_release_packages.sort(key=lambda x: x.get('createTime', ''), reverse=True)
    latest_package = xbox_release_packages[0]

    print(f"找到 {len(xbox_release_packages)} 个 Xbox release 包")
    print(f"最新版本:")
    ...
    print(f"已更新为最新 Xbox release 包: {latest_package['packageName']}")
    print()
